        ...
        <div class="mx-auto mb-4 max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-white">
                    ...
                    <form action="{{ route('game_maker_next.editnextupdate', ['prev_id' => $game_maker->next_id]) }}"  method="post" class="mt-6 space" enctype="multipart/form-data">
                        @csrf
                        <div class="bg-slate-800 text-white ">
                            @if ($game_maker->title = 'null')
                                <h1 class="mt-4">활동명 </h1>
                                <input type="text" name="title" class="input input-bordered w-full max-w-xs" value="{{ $game_maker->title }}" />

                            @else
                           <h1 class="mt-4">활동명 : {{ $game_maker->title }}</h1>
                            @endif
